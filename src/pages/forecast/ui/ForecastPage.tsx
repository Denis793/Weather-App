...
// YYYY-MM-DD in the city's local time zone
...
function buildDays(list: ForecastItem[], tz: number): DayBucket[] {
  ...
  for (const [k, itemsUnsorted] of map) {
    ...
    const vis = items
      .map((i) => (i as any).visibility as number | undefined)
    ...
    const visibilityAvg = vis.length ? Math.round(vis.reduce((s, v) => s + v, 0) / vis.length) : undefined;
  ...
  const todayKey = dayKey(nowUtc - tz, tz);
  const filtered = days.filter((d) => d.key !== todayKey);
        ...
        <div className="glass mx-auto max-w-5xl p-5 sm:p-6">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            <Metric
              icon={uiIcons.wind}
              label="Wind"
              value={`${active.windDirDominant} ${active.windAvg} m/s (max ${active.windMax})`}
            />
            <Metric
              icon={uiIcons.precip}
              label="Precip."
  ...
  return (
    <div className="rounded-xl border border-white/15 bg-white/10 px-3 py-2 backdrop-blur">
      <div className="flex items-center gap-2">
        <img src={icon} alt="" className="h-12 w-12opacity-80" />
